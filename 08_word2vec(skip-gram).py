...
with tf.device('/cpu:0'):
    ...
    embed = tf.nn.embedding_lookup(embeddings, train_inputs) # lookup table 查找结果作为输出

# Construct the variables for the NCE loss （相当于softmax那一层的权重）
...
with tf.Session() as sess:
    ...
    trained_embeddings = embeddings.eval()  # 权重矩阵
